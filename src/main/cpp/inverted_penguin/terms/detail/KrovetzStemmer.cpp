...
// TODO: Replace these with inlined functions
...
class KrovetzStemmer::Impl {
public:
  Impl(const DictTable& dictionary):
    dictEntries(dictionary), word(nullptr), j(0), k(0) {
  }

  int stem(const char* term, char* buffer, bool alreadyLowerCase);
  
private:
  const DictTable& dictEntries;
  char* word;
  int j;
  int k;
  ...
  const DictEntry *getdep(char *word);
...
/* TGA: Removed stemming cache.  It isn't thread-safe */
...
void KrovetzStemmer::kstem_add_table_entry(const char* variant, 
  ...
  int KrovetzStemmer::kstem_stem_tobuffer( const char* term, char* buffer,
					   bool alreadyLowerCase) const {
    Impl impl(dictEntries);
    return impl.stem(term, buffer, alreadyLowerCase);
  }
...
// Implementation of KrovetzStemmer::Impl
int KrovetzStemmer::Impl::stem(const char* term, char* buffer,
			       bool alreadyLowerCase) {
  int i;
  const DictEntry *dep = 0;
      
  k = (int)strlen(term) - 1;
  /* if the word is too long or too short, or not entirely
     alphabetic, just lowercase copy it into stem and return */
  /* TGA: Do not copy it.  Just return 0 and let the caller deal */
  /* TODO: Remove "'s" if present */
...
inline const KrovetzStemmer::DictEntry *KrovetzStemmer::Impl::getdep(
    char *word
) {
  const KrovetzStemmer::DictEntry *dep = 0;
  /* don't bother to check for words that are short */
  if (strlen(word) <= 1) {
    return (dep);
  } else {
    KrovetzStemmer::DictTable::const_iterator it = dictEntries.find(word);
    if (it != dictEntries.end())
      dep = &((*it).second);
  }
  return(dep);
...
inline bool KrovetzStemmer::Impl::lookup(char *word) {
  return (getdep(word) != (DictEntry *)0);
}

/* cons() returns TRUE if word[i] is a consonant. */

inline bool KrovetzStemmer::Impl::cons(int i) {
  char ch = word[i];
...
inline bool KrovetzStemmer::Impl::vowel(int i) {
  return !cons(i);
}
  

/* This routine is useful for ensuring that we don't stem acronyms */
inline bool KrovetzStemmer::Impl::vowelinstem() {
...
inline bool KrovetzStemmer::Impl::doublec (int i) {
  if (i < 1)
    return(false);
...
inline  bool KrovetzStemmer::Impl::ends(const char *str, int sufflength) {
...
inline void KrovetzStemmer::Impl::setsuff(const char *str, int length) {
...
inline  void KrovetzStemmer::Impl::plural () {
  if (final_c == 's')  {
    if (ends_in("ies")) {
      word[j+3] = '\0';
      k--;
...
inline  void KrovetzStemmer::Impl::past_tense() {
...
/* handle `-ing' endings */
inline  void KrovetzStemmer::Impl::aspect ()
  {
...
inline  void KrovetzStemmer::Impl::ion_endings () {
  int old_k = k;
...
inline void KrovetzStemmer::Impl::er_and_or_endings() {
  int old_k = k;
...
inline  void KrovetzStemmer::Impl::ly_endings () {
  int old_k = k;
...
inline void KrovetzStemmer::Impl::al_endings() {
  int old_k = k;
...
inline void KrovetzStemmer::Impl::ive_endings() {
  int old_k = k;
...
inline  void KrovetzStemmer::Impl::ize_endings() {
  int old_k = k;
...
inline  void KrovetzStemmer::Impl::ment_endings() {
  int old_k = k;
...
inline  void KrovetzStemmer::Impl::ity_endings() {
  int old_k = k;
...
inline  void KrovetzStemmer::Impl::ble_endings() {
  int old_k = k;
  char word_char;
...
inline  void KrovetzStemmer::Impl::ness_endings() {
...
inline  void KrovetzStemmer::Impl::ism_endings() {
...
inline  void KrovetzStemmer::Impl::ic_endings() {
  if (ends_in("ic")) {
...
inline  void KrovetzStemmer::Impl::ncy_endings() {
  if (ends_in("ncy"))  {
...
inline  void KrovetzStemmer::Impl::nce_endings() {
  int old_k = k;
